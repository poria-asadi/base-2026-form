...
export default async function AdminPage() {
  ...
  const db = getD1();
  const summary = await db.prepare(`SELECT COUNT(*) AS total, SUM(CASE WHEN status = 'receipt_submitted' THEN 1 ELSE 0 END) AS submitted, SUM(CASE WHEN status = 'pending' THEN 1 ELSE 0 END) AS pending FROM registrations`).first<Summary>();
  ...
  return <main className="admin-page" dir="rtl"><div className="admin-shell"><header className="admin-head"><div><span className="eyebrow">مدیریت رویداد</span><h1>ثبت‌نام‌های BASE 2026</h1><p>ورود با حساب {user.email}</p></div><div className="admin-actions"><a className={buttonVariants({ size: "lg" })} href="/api/admin/export"><Download /> دانلود فایل اکسل</a></div></header><div className={`admin-notice ${emailActive ? "active" : "pending"}`}>{emailActive ? "اعلان ایمیلی فیش‌های جدید فعال است." : "ذخیره اعلان‌ها فعال است؛ برای ارسال ایمیل، کلید سرویس ایمیل و آدرس فرستنده باید متصل شود."}</div><section className="admin-card"><div className="admin-summary"><div className="admin-stat"><span>همه ثبت‌نام‌ها</span><strong>{summary?.total ?? 0}</strong></div><div className="admin-stat"><span>فیش ثبت‌شده</span><strong>{summary?.submitted ?? 0}</strong></div><div className="admin-stat"><span>در انتظار واریز</span><strong>{summary?.pending ?? 0}</strong></div></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>نام و نام خانوادگی</th><th>شماره تماس</th><th>وضعیت</th><th>مبلغ نهایی</th><th>کد شناسایی</th><th>فیش واریزی</th><th>تاریخ ثبت</th></tr></thead><tbody>{recent.results.map((row) => <tr key={row.id}><td>{row.full_name}</td><td dir="ltr">{row.phone}</td><td>{row.status === "receipt_submitted" ? "فیش ثبت‌شده" : "در انتظار واریز"}</td><td>{row.final_amount.toLocaleString("fa-IR")} تومان</td><td dir="ltr">{row.identifier_code ?? "—"}</td><td>{row.receipt_name ? <a href={`/api/admin/receipts/${row.id}`} target="_blank" rel="noreferrer">مشاهده فیش</a> : "—"}</td><td>{new Date(row.created_at).toLocaleString("fa-IR")}</td></tr>)}</tbody></table>{!recent.results.length && <p className="admin-empty">هنوز ثبت‌نامی ثبت نشده است.</p>}</div></section></div></main>;
}
