...
import { DeleteRegistrationForm } from "@/app/admin/delete-registration-form";
...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ updated?: string; deleted?: string }> }) {
  ...
  return <main className="admin-page" dir="rtl"><div className="admin-shell">
    ...
    {deleted && <div className="admin-notice active">ثبت‌نام با موفقیت حذف شد.</div>}
    ...
    <section className="admin-card">
      ...
      <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>نام و نام خانوادگی</th><th>شماره تماس</th><th>وضعیت ثبت‌نام</th><th>مبلغ نهایی</th><th>کد شناسایی</th><th>فیش واریزی</th><th>بررسی مدیر</th><th>تاریخ ثبت</th><th>حذف</th></tr></thead><tbody>
        {recent.results.map((row) => <tr key={row.id}>
          ...
          <td><DeleteRegistrationForm registrationId={row.id} fullName={row.full_name} /></td>
        </tr>)}
