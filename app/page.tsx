...
function Field({ label, error, hint, children }: { label: string; error?: string; hint?: string; children: React.ReactNode }) {
  return <div className="field-wrap"><label className="field-label">{label}<span className="required-mark" aria-hidden="true">*</span></label>{children}{error ? <p className="field-error" role="alert">{error}</p> : hint ? <p className="field-hint">{hint}</p> : null}</div>;
}
